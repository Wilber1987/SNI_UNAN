...
class WArticlesComponent extends HTMLElement {
    ...
    DrawArticles(Dataset = this.Dataset) {       
        ...
        if (typeof Articles === "undefined" || Articles == null) {
            Articles = { type: "Articles", props: { class: this.ArticlesClass, id: "MainArticles" + this.id }, children: [] };
            Articles.children.push(this.DrawTHead());
            ...
            Articles.innerHTML = "";
            Articles.append(WRender.createElement(this.DrawTHead()));
            const tbody = this.DrawTBody(Dataset);
            if (this.paginate == true && Dataset.length > this.maxElementByPage) {
                tbody.children.forEach(tb => {
                ...
                Articles.append(WRender.createElement(tbody));
            }
                            ...
                            onchange: async (ev) => {
                                if (this.SearchItemsFromApi != undefined) {
                                    if (this.SearchItemsFromApi.Function != undefined) {
                                        ...
                                        this.DrawArticles(Dataset.data);
                                    }
                                } else {
    ...
    DrawTBody = (Dataset = this.Dataset) => {
        ...
        if (this.paginate == true && Dataset.length > this.maxElementByPage) {
            ...
            for (let index = 0; index < this.numPage; index++) {
                ...
                if (index == 0) {
                    //tBodyStyle = "display:Articles-row-group";
    ...
    DrawTFooter(tbody) {
        ...
        const SelectPage = (index) => {
            let bodys = this.shadowRoot.querySelectorAll("#MainArticles" + this.id + " tbody");
